...
const LOG_DIR = path__default.join(os__default.homedir(), ".natsuki", "logs");
if (!fs__default.existsSync(LOG_DIR)) {
  fs__default.mkdirSync(LOG_DIR, { recursive: true });
}
function getLogFile() {
  const date = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  return path__default.join(LOG_DIR, `natsuki-${date}.jsonl`);
}
function logEvent(event, payload) {
  ...
  try {
    fs__default.appendFileSync(getLogFile(), JSON.stringify(entry) + "\n");
  } catch (e) {
...
function setIdleCallback(callback) {
...
function startSession(cwd) {
  ...
  const shell = os__default.platform() === "win32" ? "cmd.exe" : "bash";
  const targetCwd = cwd || os__default.homedir();
  const claudeCommand = os__default.platform() === "win32" ? "claude" : "claude";
...
function setupPty(win2) {
  builder.on("data", (data) => {
    if (!win2.isDestroyed()) win2.webContents.send("terminal-data", data);
  });
  builder.on("exit", (info) => {
    if (!win2.isDestroyed()) win2.webContents.send("terminal-exit", info);
  });
  builder.on("error", (err) => {
    if (!win2.isDestroyed()) win2.webContents.send("terminal-data", `\r
Error: ${err}\r
`);
  });
  ...
  ipcMain.on("terminal-init", (_event, cwd) => {
    startSession(cwd);
  });
...
function setupGitHandlers() {
  ...
  ipcMain.handle("git-diff", async (_event, { cwd, file }) => {
    try {
  ...
  ipcMain.handle("git-restore", async (_event, cwd) => {
    try {
...
const SNAPSHOT_BASE_DIR = path$1.join(os.homedir(), ".natsuki", "snapshots");
...
async function runVerify(cwd, completionProfile) {
  ...
  return {
    success: exitCode === 0,
    exitCode,
    stdoutTail: stdout.slice(-2e3),
    stderrTail: stderr.slice(-2e3)
  };
}
async function createSnapshot(cwd, jobId, intent = "") {
  try {
    const timestamp = /* @__PURE__ */ new Date();
    const snapshotId = timestamp.toISOString().replace(/[:.]/g, "-");
    const snapshotDir = path$1.join(SNAPSHOT_BASE_DIR, jobId, snapshotId);
    ...
    await fs.promises.writeFile(path$1.join(snapshotDir, sources.git_status), gitStatus);
    await fs.promises.writeFile(path$1.join(snapshotDir, sources.git_diff), gitDiff);
    await fs.promises.writeFile(path$1.join(snapshotDir, sources.terminal_tail), terminalLines.join("\n"));
    const manifest = {
    ...
    await fs.promises.writeFile(path$1.join(snapshotDir, "manifest.json"), JSON.stringify(manifest, null, 2));
...
function setupSnapshotHandlers(win2) {
  ipcMain.handle("verify-run", async (_event, { cwd, jobId, profile }) => {
    return await runVerify(cwd, profile);
  });
  ipcMain.handle("snapshot-create", async (_event, { cwd, jobId, includeVerify }) => {
...
const SECRETS_FILE = path$1.join(os.homedir(), ".natsuki", "secrets.json");
class KeyManager {
  storePath;
  ...
  ensureStoreExists() {
    const dir = path$1.dirname(this.storePath);
    if (!fs.existsSync(dir)) {
...
Git Diff:
\`\`\`diff
${snapshot.diff.slice(0, 15e3)}
\`\`\`
${snapshot.diff.length > 15e3 ? "(Truncated: original was " + snapshot.diff.length + " chars)" : ""}

Recent Logs:
\`\`\`
${snapshot.logs.slice(-3e3)}
\`\`\`
...
async function loadSnapshot(jobId, snapshotId) {
  const snapshotDir = path$1.join(os.homedir(), ".natsuki", "snapshots", jobId, snapshotId);
  let diffContent = "", logContent = "", manifestStr = "";
  try {
    diffContent = await fs.promises.readFile(path$1.join(snapshotDir, "git_diff.patch"), "utf-8");
    logContent = await fs.promises.readFile(path$1.join(snapshotDir, "terminal_tail.txt"), "utf-8");
    manifestStr = await fs.promises.readFile(path$1.join(snapshotDir, "manifest.json"), "utf-8");
  } catch (e) {
...
function setReviewerProvider(provider) {
  if (reviewers[provider]) {
    defaultProvider = provider;
    console.log(`[LLM] Reviewer provider set to ${provider}`);
  }
}
async function runReview(jobId, snapshotId, apiKey) {
  try {
...
function setupLLMHandlers(win2) {
  ipcMain.handle("review-run", async (_event, { jobId, snapshotId, apiKey }) => {
    return await runReview(jobId, snapshotId, apiKey);
  });
  ipcMain.handle("set-card-provider", (_event, provider) => {
...
class Orchestrator {
  ...
  setupIPC() {
    ipcMain.handle("orchestrator-start", async (_, { jobId, cwd }) => {
      return this.startJob(jobId, cwd);
    });
    ipcMain.handle("orchestrator-step", async (_, { jobId }) => {
      return this.advanceLoop(jobId);
    });
    ipcMain.handle("orchestrator-proceed", async (_, { jobId, action }) => {
  ...
  async startJob(jobId, cwd) {
    ...
    const job = {
      ...
      history: [],
      autoFixCount: 0,
      apiKey: hasKey ? "present" : void 0
      // Keeping field name for frontend compatibility if needed, but value is dummy
    };
  ...
  async advanceLoop(jobId) {
    ...
    try {
      if (job.status === "running" || job.status === "fixing") {
        ...
        const verifyRes = await runVerify(job.workspace, "lint");
        job.history.push({ timestamp: Date.now(), action: "verify", result: verifyRes });
        this.updateJobStatus(jobId, "snapshotting");
        const snapRes = await createSnapshot(job.workspace, jobId, job.description);
        ...
        if (canReview) {
          ...
          const reviewRes = await runReview(jobId, snapRes.snapshotId, "");
  ...
  async handleReviewDecision(jobId, result) {
    const job = activeJobs.get(jobId);
    if (!job) return;
...
Please fix these issues to reach the next level.`;
  ...
  async handleUserAction(jobId, action) {
    const job = activeJobs.get(jobId);
    if (!job) return;
  }
}
function setupOrchestrator(win2) {
  new Orchestrator(win2);
}
const DOMAIN_LEVELS = {
  "game": {
...
path$1.join(os.homedir(), ".natsuki", "research");
async function generatePremises(intent) {
...
async function generatePlan(premises) {
  const checklist = [
...
async function collectEvidence(cwd, plan) {
  const kbDir = path$1.join(cwd, "knowledge");
  const evidences = [];
  if (fs.existsSync(kbDir)) {
    try {
      ...
      for (const file of files) {
        if (file.endsWith(".md")) {
          const content = await fs.promises.readFile(path$1.join(kbDir, file), "utf-8");
...
async function buildContract(premises, evidence) {
...
function setupResearchHandlers() {
  ipcMain.handle("research-intent", async (_, intent) => {
    return await generatePremises(intent);
  });
  ipcMain.handle("research-plan", async (_, premises) => {
    return await generatePlan(premises);
  });
  ipcMain.handle("research-evidence", async (_, { cwd, plan }) => {
    return await collectEvidence(cwd, plan);
  });
  ipcMain.handle("research-build-contract", async (_, { premises, evidence }) => {
...
process.env.DIST = path$2.join(__dirname$1, "../dist");
process.env.VITE_PUBLIC = app.isPackaged ? process.env.DIST : path$2.join(process.env.DIST, "../public");
let win;
...
app.whenReady().then(() => {
  ...
  if (win) {
    ...
    ipcMain.handle("key-set", async (_event, provider, key) => {
      return await keyManager.setApiKey(provider, key);
    });
    ipcMain.handle("key-has", (_event, provider) => {
      return !!keyManager.getApiKey(provider);
    });
    ipcMain.handle("key-delete", (_event, provider) => {
